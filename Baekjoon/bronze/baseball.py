...
# 문서에는 회차별 양 팀 획득 점수만이 적혀져 있기 때문이다
# 경근이는 주어진 방대한 자료를 해석하는 것이 귀찮았기 때문에 절친한 프로그래머 친구에게 도움을 요청했다
# 주어진 실황 기록문서에서 어떤 팀이 이겼느지 알아내자

# 입력 파일의 첫 번째 줄에 테스트 케이스의 수를 의미하는 자연수 T가 주어진다
# 그 다음에는 T개의 테스트 케이스가 주어진다
# 각 테스트 케이스는 9줄에 걸쳐 입력되며 매 줄마다 해당 회의 연세대 득정 Y와
# 고려대 득점 K가 공백으로 구분되어 주어진다
