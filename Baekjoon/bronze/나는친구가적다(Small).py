...
# F 학점을 받을 위기까지 아슬아슬하게 결석일수를 유지하던 상민이는
# 어느날 갑자기 영문도 모른채 쪽지시험을 보게 되었다
...
# 하지만 성민이에겐 큰 문제가 생겼다. 결석한 날의 수업 내용을 친구에게 빌려
# 필기를 하던 중 교과서에 숫자(0-9)를 적어버린 것이다
# 키워드를 찾기 힘들어 패닉에 빠진 성민이는 몇 안 되는 친구인 당신에게 도움을 요청했다
# 성민이를 도와 교과서에서 성민이가 찾고자 하는 키워드의 존재 여부를 알려주자

# 첫번째 줄에는 알파벳 소문자, 대문자, 숫자로만 이루어진 문자열 S가 주어진다
# 두번째 줄에는 성민이가 찾고자 하는 알파벳 소문자, 대문자로만 이루어진 키워드 문자열 K가 주어진다
# 단 입력으로 들어오는 키워드 문자열 K의 길이는 교과서의 문자열 S보다 짧거나 같다

# 첫번째 줄에 성민이가 찾고자 하는 키워드가 문자열 내에 존재하면 1
# 존재하지 않으면 0을 출력
...
S = input()
K = input()
string = re.findall(K, re.sub('[0-9]', '', S))
print(1 if string else 0)
