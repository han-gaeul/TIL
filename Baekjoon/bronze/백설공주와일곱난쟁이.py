...
# 백설공주는 이런 일이 생길 것을 대비해서 난쟁이가 쓰고 다니는 모자에
# 100보다 작은 양의 정수를 적어 놓았다. 사실 백설 공주는
# 공주가 되기 전에 매우 유명한 수학자였다. 따라서, 일곱 난쟁이의
# 모자에 쓰여 있는 숫자의 합이 100이 되도록 적어 놓았다
# 아홉 난쟁이의 모자에 쓰여 있는 수가 주어졌을 때 일곱 난쟁이를 찾아라
...
li = [int(input()) for _ in range(9)]
result = [i for i in itertools.combinations(li, 7) if sum(i) == 100]
print(*result[0], sep='\n')