...
# 토마토의 영향을 받아 익게 된다. 하나의 토마토에 인접한 곳은
...
# 둘째 줄부터는 가장 밑의 상자부터 가장 위의 상자까지에
# 저장된 토마토들의 정보가 주어진다. 즉, 둘째 줄부터 N개의 줄에는
# 하나의 상자에 담긴 토마토의 정보가 주어진다
# 각 줄에는 상자 가로줄에 들어있는 토마토들의 상태가 M개의 
# 정수로 주어진다. 정수 1은 익은 토마토, 정수 0은 익지 않은 토마토,
    ...
    for z in range(H):
        for y in range(N):
            for x in range(M):
                if box[z][y][x] == 1:
                    q.append((x, y, z, 0))
    # 큐에서 하나씩 꺼내서 인접한 안익은 토마토들을 익게 함
    while q:
